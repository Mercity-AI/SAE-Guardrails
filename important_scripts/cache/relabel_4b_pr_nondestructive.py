#!/usr/bin/env python3
"""Build a PROMPT+RESPONSE-supervised copy of the 4B sae1500 cache (non-destructive).

Mirrors relabel_1b_prompt_response.py but for the Gemma-3-4B backbone and WITHOUT clobbering the
original response-only cache: it creates cache/sae1500_10k_gemma4b_pr/ that symlinks features/
lengths/roles/split/selection from the response-only cache and writes a fresh prompt+response
labels.npy there. Uses the shared labeler (label_record_pr) so the convention matches exactly.

  python relabel_4b_pr_nondestructive.py
"""
...
SRC = ROOT / "cache/sae1500_10k_gemma4b_prompt_response"
OUT = ROOT / "cache/sae1500_10k_gemma4b_pr"
LINK = [
    "features.npy",
    "lengths.npy",
    "role_ids.npy",
...
def main() -> None:
    """Relabel the 4B response-only cache to prompt+response and write the non-destructive PR copy."""
