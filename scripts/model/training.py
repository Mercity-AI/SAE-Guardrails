...
from important_scripts.model.utils import (
    PAD,
    TOPICS,
    build_model,
    collate,
...
def verify_cache_contract(config: dict, cache: Path) -> None:
    """Check the YAML's data claims against what the cache on disk actually contains.

    ``load_config`` only checks that the configuration *states* the corrected-run conventions.
    A wrong ``cache_dir`` with the right strings would pass, so verify the two claims that are
    checkable from the cache itself: the declared feature width, and whether prompt tokens are
    really supervised. Mismatches here mean the run would silently train on a different cache
    than the configuration describes.
    """
    features = np.load(cache / "features.npy", mmap_mode="r")
    declared_width = int(config["data"]["input_features"])
    if features.shape[1] != declared_width:
        raise ValueError(
            f"{cache}/features.npy is {features.shape[1]} wide but data.input_features says "
            f"{declared_width}; the configuration does not describe this cache"
        )
    supervision = str(config["data"].get("supervision", ""))
    wants_prompt = supervision.startswith("prompt_and_response")
    labels = np.load(cache / "labels.npy", mmap_mode="r")
    roles = np.load(cache / "role_ids.npy", mmap_mode="r")
    prompt_labeled = int(((np.asarray(labels) != PAD) & (np.asarray(roles) != 2)).sum())
    if wants_prompt and not prompt_labeled:
        raise ValueError(
            f"data.supervision says {supervision!r} but {cache}/labels.npy has no labeled prompt "
            "tokens; this is a response-only cache"
        )
    if not wants_prompt and prompt_labeled:
        raise ValueError(
            f"data.supervision says {supervision!r} but {cache}/labels.npy has {prompt_labeled} "
            "labeled prompt tokens; this is a prompt+response cache"
        )
    print(
        f"cache contract verified: width={features.shape[1]} "
        f"prompt_labeled_tokens={prompt_labeled} supervision={supervision!r}",
        flush=True,
    )
...
def run(config_path: Path, allowed_architectures: set[str] | None = None):
    ...
    require_cache(cache)
    verify_cache_contract(config, cache)
    ...
    if gpu_resident_cache:
        ...
        data = [
            (
                torch.as_tensor(np.asarray(features, dtype=np.float16)).to(device),
